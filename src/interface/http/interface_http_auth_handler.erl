...
-module(interface_http_auth_handler).
...
init ( Request=#{method := <<"POST">>, bindings := #{action := <<"login">>}, has_body := true }, State ) ->  
        ...
        #{ <<"username">> := Username, <<"password">> := Password } ->
            case erlstore_persistence:get ( users, Username ) of
                ...
                {2000, User=#{<<"id">> := Id, <<"password">> := Hash, <<"salt">> := Salt } } ->
                    case interface_auth:credentials ( check, {Username, Id}, {Password, Salt, Hash} ) of
                        true ->
                            {2000, User};
                        false ->
                            {5000, null}
    ...
    case Credentials of
        {2000, VerifiedUser} ->
            Token = interface_auth:token ( generate, VerifiedUser, <<"secretkey">>, 900 ),
